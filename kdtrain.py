...
import BNNtest as test  # import test.py to get mAP after each epoch
from BNNmodels import *
import models
...
def train():
    ...
    tweights = opt.tweights  # initial teacher weights
    ...
    tmodel = models.Darknet(tcfg, arc=opt.arc).to(device)
    # Optimizer
    ...
    attempt_download(tweights)
    if tweights.endswith('.pt'):  # pytorch format
        # possible tweights are 'last.pt', 'yolov3-spp.pt', 'yolov3-tiny.pt' etc.
        if opt.bucket:
            os.system('gsutil cp gs://%s/last.pt %s' % (opt.bucket, last))  # download from bucket
        tchkpt = torch.load(tweights, map_location=device)

        # load tmodel
        # if opt.transfer:
        tchkpt['model'] = {k: v for k, v in tchkpt['model'].items() if tmodel.state_dict()[k].numel() == v.numel()}
        tmodel.load_state_dict(tchkpt['model'], strict=False)
        # else:
        #    tmodel.load_state_dict(tchkpt['model'])
        # load optimizer
        if tchkpt['optimizer'] is not None:
            optimizer.load_state_dict(tchkpt['optimizer'])
            best_fitness = tchkpt['best_fitness']
        # load results
        if tchkpt.get('training_results') is not None:
            with open(results_file, 'w') as file:
                file.write(tchkpt['training_results'])  # write results.txt

        start_epoch = tchkpt['epoch'] + 1
        del tchkpt

    elif len(tweights) > 0:  # darknet format
        # possible weights are 'yolov3.weights', 'yolov3-tiny.conv.15',  'darknet53.conv.74' etc.
        cutoff = load_darknet_weights(tmodel, tweights)
    ...
    if opt.transfer or opt.prebias:  # transfer learning edge (yolo) layers
        nf = int(tmodel.module_defs[tmodel.yolo_layers[0] - 1]['filters'])  # yolo layer size (i.e. 255)
    ...
    if mixed_precision:
        tmodel, optimizer = amp.initialize(tmodel, optimizer, opt_level='O1', verbosity=0)
    ...
    if torch.cuda.device_count() > 1:
        ...
        tmodel = torch.nn.parallel.DistributedDataParallel(tmodel)
        tmodel.yolo_layers = tmodel.module.yolo_layers  # move yolo layer indices to top level
    # 获得要剪枝的层
    ...
    # Start training
    tmodel.nc = nc  # attach number of classes to model
    tmodel.arc = opt.arc  # attach yolo architecture
    tmodel.hyp = hyp  # attach hyperparameters to model
    # model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device)  # attach class weights
    torch_utils.model_info(model, report='summary')  # 'full' or 'summary'
    torch_utils.model_info(tmodel, report='summary')  # 'full' or 'summary'
    nb = len(dataloader)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            kdloss = compute_lost_KD(pred, tpred)
            loss = 0.9 * loss + 0.1 * kdloss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tcfg', type=str, default='cfg/yolov3-hand.cfg', help='tcfg file path')
    ...
    parser.add_argument('--tweights', type=str, default='weights/yolov3.weights',
                        help='initial tweights')  # i.e. weights/darknet.53.conv.74
